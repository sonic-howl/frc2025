...
class Config:
    class MAXSwerveModule:
        ...
        driveConfig.setIdleMode(SparkBaseConfig.IdleMode.kBrake).smartCurrentLimit(50).inverted(
            False
        )
        ...
        ).pid(0.1, 0, 0).velocityFF(drivingVelocityFeedForward).outputRange(-1, 1)
        ...
        ).pid(1, 0, 0).outputRange(-1, 1).positionWrappingEnabled(True).positionWrappingInputRange(
            0, kTurningFactor
        )
